...
async function sendMessage() {
    ...
    try {
        const apiUrl = window.SkillTechAPI_URL || '/api/aichat';
...
function formatText(text) {
    ...
    // 7. Format Markdown Headers
    clean = clean.replace(/^### (.*?)$/gm, '<h3 style="font-size:16px; font-weight:700; margin:10px 0 5px 0; color:#263B56;">$1</h3>');
    clean = clean.replace(/^## (.*?)$/gm, '<h2 style="font-size:18px; font-weight:700; margin:12px 0 6px 0; color:#263B56;">$1</h2>');
    clean = clean.replace(/^# (.*?)$/gm, '<h1 style="font-size:20px; font-weight:700; margin:14px 0 7px 0; color:#263B56;">$1</h1>');

    // 8. Format Bullet Points (lines starting with ✓, •, *, or -)
